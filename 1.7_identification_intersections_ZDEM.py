...
for layer_line, layer_plate in corres_layers_rev.items():
    ...
    mask_zdem = gdf_line["PLATE"].astype(str).str.strip().str.upper().isin(["ZDEM", "Z_DEM"]) # Prend en compte également si il y a des espaces et majuscules
...
# Resumé statistique du nombre de plaques intersectées : 

report_stats_out = r"C:\PANALESIS\Report\report_ZDEM_check_intersection_stat.txt"

stats = (report_df.groupby(["layer", "nb_plaques"]).size().reset_index(name="nb_lignes"))

with open(report_stats_out, "w", encoding="utf-8") as stat:

    stat.write("Intersections Lignes ZDEM et Z_DEM avec plaques\n\n")

    for layer in stats["layer"].unique():

        stat.write(f"Couche : {layer}\n")

        layer_stats = stats[stats["layer"] == layer]

        total = layer_stats["nb_lignes"].sum()

        stat.write(f"Total ZDEM : {total}\n")

        for _, row in layer_stats.iterrows():

            stat.write(f"{row['nb_plaques']} plaques : "f"{row['nb_lignes']} lignes\n")
